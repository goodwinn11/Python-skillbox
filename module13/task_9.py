...
def payments(credit, duration, rate, refinance):
  k = rate * (1 + rate) ** duration / ((1 + rate) ** duration - 1)
  ...
  if not refinance:
    ...
    n = duration + 1
  for i in range(1, n):
    percent = credit_balance * rate
    print("\nПериод", i)
    print("Остаток долга на начало периода", credit_balance)
    print("Выплачено процентов", percent)
    print("Выплачено от тела долга", credit * k - percent)
    credit_balance -= credit * k - percent
  return credit_balance

credit = float(input("Введите сумму кредита: "))
duration = int(input("На сколько лет выдан? "))
rate = int(input("Сколько процентов годовых? "))
refinance = False
rate /= 100
credit_balance = credit
credit_balance = payments(credit, duration, rate, refinance)
print("Остаток долга", credit_balance)
duration -= 3
refinance = True
duration += int(input("\nНа сколько лет продляется договор? "))
rate = int(input("До скольки процентов увеличивается ставка? "))
rate /= 100
credit_balance = payments(credit_balance, duration, rate, refinance)
print("Остаток долга", credit_balance)