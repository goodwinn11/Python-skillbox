...
safety = float(input("Введите максимально допустимый уровень опасности: "))
x = 0
min_x = 0
max_x = 4
mid_x = (min_x + max_x) / 2
while abs(danger(x)) > safety:
  if abs(danger((min_x + mid_x) / 2)) <= abs(danger((max_x + mid_x) / 2)):
    max_x = mid_x
    mid_x = (min_x + max_x) / 2
    x = mid_x
  else:
    ...
    mid_x = (min_x + max_x) / 2
    x = mid_x
print(f"Приблизительная глубина кладки равна {x} м")
